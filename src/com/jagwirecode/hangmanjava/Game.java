...
public class Game {
     ...
     * the words.txt file located in the res folder for words it can scan from. It then loops
     * through the file, adding each word to a list of words, for which it then randomly selects
     * from. </p>
     *
     ...
     * For letters, the HashMap has their value updated to NOT_GUESSED which is used when printing
     * the word in guessString.</p>
     */
    ...
    /**
     * <p>Checks input (used as key) based on HashMap value tied to it, such that the guess can
     * be checked to see if it's in the word or not, or it has been previously guessed.</p>
     *
     * <p>The input is checked to be a single letter, otherwise the check is aborted. If the value
     * of the key is NOT_GUESSED, it means it is in the word and hasn't been guessed yet, and as
     * such the value is updated to reflect a correct guess. If the value is NOT_IN_WORD, the guess
     * is incorrect, and the value is updated to reflect an incorrect guess. If the value is
     * GUESSED_SUCCESS or GUESSED_WRONG, the guess is discarded as it's already been guessed, and
     * the user is informed.</p>
     *
     * @param guess - String input by the user.
     * @return GuessResult - enum that has multiple values to reflect the state of the guess.
     *        See {@link GuessResult} for more info.
     */
    ...
    /**
     * Loops through word, printing letters for previously correctly guessed characters and
     * underscores otherwise.
     *
     * <p>A for loop goes through the word as a char array and checks the value of the each
     * letter (key). If the value is GUESSED_SUCCESS, the letter is printed. If it is NOT_GUESSED,
     * an underscore is printed. Any other item in the word is printed as is (spaces, apostrophes
     * etc)</p>
     */
    ...
    /**
     * A set of every entry in the HashMap is produced, where an entry is printed if it has been
     * guessed incorrectly.
     *
     * <p>The set is looped through, with each key-value pair being checked for GUESSED_WRONG, where
     * it is printed if present.</p>
     */
    ...
    /**
     * <p>A set of every entry in the HashMap is produced, if any entry has NOT_GUESSED as its
     * value, the word has not been completed and as such is not finished.</p>
     *
    ...
    /**
     * <p>Provides a com.jagwirecode.hangmanjava.Game instance with the ability to manage the status
     * of guessed letters etc.</p>
     */
